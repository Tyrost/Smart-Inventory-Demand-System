...
from utils.misc import warehouses
...
def thread_restock(data:List[dict])->pd.DataFrame:
    ...
    try:
        ...
        processed = processed.rename(columns={"adjusted_allocation": "quantity_change"})
...
def map_restock(date, database:Commander, subset=100)->None: # take sample of the last 100 as default from the database (?)
    ...
    accurate the result will be. In this case the accuracy of our trend detection. For the purpose of efficiency, we will stick
    with a default value of 100.
    
    This function will restock products based on sales and keep track in the `inventory_log` table.
    '''
